...
async def return_from_temp(db, seat_id: int):
    ...
    # 송도균이 넣은 코드 - 센서 상태 업데이트 로직
    
async def update_sensor_status(db: AsyncSession, seat_id: int, occupied: bool):
    seat = await get_seat(db, seat_id)

    if not seat:
        return None

    seat.sensor_occupied = occupied
    seat.is_occupied = occupied

    if occupied:
        # 센서가 착석 감지하면 사용중으로 변경
        seat.status = SeatStatusEnum.using
        seat.vacant_since = None
    else:
        # 센서가 미착석 감지
