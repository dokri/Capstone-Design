...
def _calc_vacant_seconds(seat, now: datetime):
    """
    1. 상태가 vacant(비어있음)이고
    2. 예약 중(is_booked=True)인 경우에만 
    비어있는 경과 시간을 계산합니다.
    """
    # getattr를 사용하여 DB에 컬럼이 아직 없어도 에러가 나지 않게 안전하게 처리
    is_booked = getattr(seat, 'is_booked', False)
    
    if is_booked and seat.status == SeatStatusEnum.vacant and seat.vacant_since is not None:
...
# _to_seat_status 함수에서 is_booked 정보도 넘겨주도록 schemas.py와 함께 수정 필요


def _to_seat_status(seat, now: datetime) -> SeatStatus:
    # SeatStatus 스키마에 is_booked 필드가 있다면 추가해주는 것이 좋습니다.
    return SeatStatus(
        id=seat.id,
        name=seat.name,
        status=seat.status,
        is_booked=getattr(seat, 'is_booked', False),  # 이 필드를 추가해서 웹에 전달
...
async def periodic_seat_release(interval: int = 10):
    """
    10초마다 실행되며, 예약 중인데 너무 오래 비어있는(노쇼) 좌석을 
    자동으로 vacant 및 예약 해제 처리합니다.
    """
    while True:
        await asyncio.sleep(interval)
        async with AsyncSessionLocal() as db:
            # 이 함수(release_timed_out_seats) 내부에서도 is_booked=False 처리가 필요합니다.
...
# main.py 수정
@app.get("/admin/cameras", response_model=List[schemas.CameraResponse]) # CameraResponse 사용!
async def read_all_cameras(db: AsyncSession = Depends(get_db)):
    cameras = await crud.get_all_cameras(db)
    return cameras

@app.get("/admin/cameras/{camera_id}", response_model=schemas.CameraResponse) # CameraResponse 사용!
async def read_camera(camera_id: str, db: AsyncSession = Depends(get_db)):
    camera = await crud.get_camera(db, camera_id)
    if not camera:
        raise HTTPException(status_code=404, detail=f"ID가 '{camera_id}'인 카메라를 찾을 수 없습니다.")
    return camera
