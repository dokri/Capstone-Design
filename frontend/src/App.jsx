...
const convertSeatGroupsToSeats = (seatGroups) => {
  ...
  return seatGroups.flatMap((group) =>
    (group.seats || []).map((seat) => ({
      ...
      camera_id: group.camera_id || seat.camera_id,
      coord_x: seat.coord_x,
      coord_y: seat.coord_y,
      vacant_seconds: seat.vacant_seconds,
    }))
  );
};

const mergeApiSeatsWithBaseSeats = (baseSeats, apiSeats) => {
  const safeBaseSeats =
    Array.isArray(baseSeats) && baseSeats.length >= 70
      ? baseSeats
      : createInitialSeats();

  return safeBaseSeats.map((baseSeat) => {
    const matchedSeat = apiSeats.find(
      (apiSeat) => Number(apiSeat.id) === Number(baseSeat.id)
    );
...
const App = () => {
  const [seats, setSeats] = useState(() => {
    ...
    if (savedSeats) {
      ...
      return parsedSeats.length >= 70 ? parsedSeats : createInitialSeats();
    }
  ...
  const fetchSeats = async () => {
    try {
      ...
      if (apiSeats.length > 0) {
        setSeats((prevSeats) =>
          mergeApiSeatsWithBaseSeats(prevSeats, apiSeats)
        );
      } else {
        // 추가 : 서버 DB가 비어 있으면 프론트 예약/좌석 상태도 초기화
