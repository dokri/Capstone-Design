...
def _to_seat_status(seat, now: datetime) -> SeatStatus:
    return SeatStatus(
        ...
        # 추가: 관리자페이지 새로고침 후에도 X/Y 좌표가 유지되도록 응답에 좌표 포함
...
async def receive_detections(
    ...
    return {
        "processed": len(payload.detections),
        "seats_updated": len(seats),
    }
...
async def get_all_seats(db: AsyncSession = Depends(get_db)):
    ...
    for camera in cameras:
        seats_result = await db.execute(
            select(models.Seat).where(models.Seat.camera_id == camera.id)
        )
...
async def get_seat_status(camera_id: str, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(models.Camera).where(models.Camera.id == camera_id)
    )
    ...
    seats_result = await db.execute(
        select(models.Seat).where(models.Seat.camera_id == camera_id)
    )
    seats = seats_result.scalars().all()
...
async def update_homography(
    camera_id: str,
    payload: HomographyUpdate,
    db: AsyncSession = Depends(get_db),
):
    ...
    try:
        matrix = compute_homography(
            [{"src": p.src, "dst": p.dst} for p in payload.points]
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
